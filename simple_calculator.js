...
//STEP 5(a): fetch out any of this operator +/-*
        let operator
//This is the list of Not a number (e.g ".", ".", "*",)
        let NaNList = []
        ...
        operand.forEach(element => {
            ...
            if (isNaN(element)) {
             NaNList.push( element )//expected output is ".", "*", "."   from EXAMPLE 1
            }
        });

        operator = NaNList.sort()[0] //expected output after sorting is "*", ".", "."
...
//step 5(c) and (d)
        ...
        let firstNumber = parseFloat(operand.slice(0, operatorIndex).join(""))
        let secondNumber = parseFloat(operand.slice(operatorIndex +1, operand.length).join(""))
...
//STEP 6(b)
        ...
        if(operation === "√") {
            /* if(!isNaN(firstOperand)){
             answer = squareRoot(firstOperand)
             input_field.value = `${operation}${firstOperand} = ${answer}`
             } */
            if(!isNaN(secondOperand)){
                answer = squareRoot(secondOperand)
                input_field.value = `${operation} ${secondOperand} = ${answer}`
            }
        }
        else{
